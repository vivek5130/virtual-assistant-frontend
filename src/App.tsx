...
function ShoppingAssistant({
  ...
  const handleAddToCart = (product: Product) => {
    ...
    const alreadyInCart = storedCart.find(
  ...
  const quickQuestions: QuickQuestion[] = [
    {
      ...
      text: "Track my order",
      response:
        "To track your order, please provide your order number and I can help you check its status.",
    },
    {
      id: 2,
      text: "Return policy",
    ...
    },
    {
      ...
      response:
        "We accept UPI Payments and all major credit cards, PayPal and Apple Pay.",
    },
    {
      id: 4,
      text: "Find products",
      response:
        "I'll help you find the perfect products! Let me ask you a few questions.",
    },
  ];
  ...
  const getProductRecommendations = async (context: string[]) => {
    try {
      const productsContext = products
        .map(
          (p) => `${p.name} (${p.category}) - $${p.price} - ${p.description}`
        )
        .join("\n");
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (nextResponse.startsWith("COMPLETE:") || questionCount >= 2) {
        const { text, recommendedProducts } = await getProductRecommendations(
          newContext
        );
